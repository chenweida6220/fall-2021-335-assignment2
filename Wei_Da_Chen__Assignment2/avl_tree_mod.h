/*
  Name: Wei Da Chen
  Date: 10/11/2021
	Header file: avl_tree_mod.h
*/

/*
This file includes the AvlTree class, a modified AVL implementation which uses double
rotations directly instead of calling 2 single rotations.
*/
...
class AvlTree {
  public:
    ...
    // gets the sequences by the use of a helper function
    void getSeq(const string &x) const {
    ...
    // returns the total # of nodes using a helper function
    int totalNodes() const {
      return totalNodes_helper(root);
    }

    // returns the depth of the nodes using a helper function
    int depthOfTree( int depth ) {
      return depthOfTree_helper(root, depth);
    }
  ...
  private:
    ...
    void getSeq(const string & x, AvlNode *t) const {
        if (t == nullptr) {
            cout << "Not Found" << endl;
        }
        else if(x < t->element.getRecSequence()) {
            return getSeq(x, t->left);
        }
        else if (t->element.getRecSequence() < x) {
            return getSeq(x, t->right);
        }
        else {
            t -> element.printEnzAcronyms();
        }
    }

    // returns the total number of nodes of the tree
    int totalNodes_helper( AvlNode *t ) const {
        int count = 0;
        if( t != nullptr ) {
            count = 1 + totalNodes_helper( t->left ) + totalNodes_helper( t->right );
        }
        return count;
    }

    // returns the # of child nodes for t
    int depthOfTree_helper(AvlNode *t, int depth) {
      if (t == nullptr)
          return 0;
      else
          return depth + depthOfTree_helper(t->left, depth+1) + depthOfTree_helper(t->right, depth+1);
    }

    // finds and prints the element in t that has the same value as x
    void printHelper( const Comparable &x, AvlNode *t ) {
        if( t == nullptr )
            return;
        else if( x < t->element )
